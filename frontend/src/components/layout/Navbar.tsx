  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-colors duration-300">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        {/* Logo */}
        <Link to="/" className="text-xl font-bold bg-linear-to-r from-blue-600 to-indigo-500 bg-clip-text text-transparent">
          CodeBlog
        </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative group flex items-center gap-1 text-slate-600 dark:text-slate-300">
            ...
            >
              <option value="uz" className="dark:bg-slate-800">UZ</option>
              <option value="en" className="dark:bg-slate-800">EN</option>
              <option value="ru" className="dark:bg-slate-800">RU</option>
            </select>
          </div>
          ...
          {isAuthenticated && user ? (
            <div className="flex items-center gap-3 border-l pl-4 border-slate-200 dark:border-slate-750">
              <Link to="/dashboard" className="flex items-center gap-2 group">
                <img src={user.avatarUrl} alt={user.fullName} className="w-8 h-8 rounded-full object-cover ring-2 ring-blue-500/20" />
